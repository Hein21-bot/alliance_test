...
export default class ForeignerSalaryTable extends Component {
  ...
  handleSearch(
    ...
    fetch(
      `${main_url}foreigner_salary/get_foreigner_salary/${
        selected_branch.value
      }/${selected_region.value}/${selected_dept.value}/${moment(
        requestMonth
      ).format("YYYY-MM")}/${user_id}`
    )
      .then((response) => {
  ...
  _setTableData = (data) => {
    ...
    if (data) {
      for (var i = 0; i < data.length; i++) {
        ...
        obj = {
          ...
          deductionOrAddition: data[i].deduction_or_addition
            ? data[i].deduction_or_addition
            : "-",
          salaryAfterDOrA: data[i].after_deduction_or_addition
          ...
          housing_allowance: data[i].housing_allowance
            ? data[i].housing_allowance
            : "-",
          total_gross_salary: data[i].total_gross_salary
            ? data[i].total_gross_salary
            : "-",
          ...
          debut_for_motorbike: data[i].debut_for_motorbike
            ? data[i].debut_for_motorbike
            : "-",
          deduction_of_loan: data[i].deduction_of_loan
            ? data[i].deduction_of_loan
            : "-",
    ...
    var column = [
      ...
      {
        title: "Deduct for Office Motorbike Using",
        data: "debut_for_motorbike",
      },
  ...
  render() {
    console.log('this.props.regionList =====>', this.props.regionList)
    return (
      <div>
        <div className="row" style={{ display: "flex", alignItems: "end" }}>
          ...
          <div className="col-md-2">
            ...
            <Select
              options={this.props.deptList}
              value={this.state.selected_dept}
