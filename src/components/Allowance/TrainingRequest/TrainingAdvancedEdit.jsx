...
import {
    getUserId, main_url, getTrainingVenue, validate, getMainRole, getActionStatus, havePermission,
...
export default class TrainingAdvancedEdit extends Component {
    constructor(props) {
        super(props);
        this.state = {
    ...
    check = () => {
        ...
        if (validate('check_form')) {
            ...
            let info = [this.state.data].map(v => ({ ...v, status: v.status == 5 ? 0 : v.status }))[0];
            let detail = this.state.detail;
            let expense = this.state.expense;
            let grand = this.state.data.totalTrainingCost + this.state.data.totalExpense;;
            let avgCost = avgCostPerPerson.toFixed(2);
            var { status_title, is_main_role } = this.state;


            if (status_title !== '' && is_main_role) {
                var action = getActionStatus(status_title, this.state.data, this.state.user_id);
    ...
    changeText = (e) => {
        ...
        if (name === 'noOfParticipant') {
            ...
            cost = Number(data.noOfParticipant) * this.state.newData.costFee
    ...
    render() {
        ...
        avgCostPerPerson = (this.state.data.totalTrainingCost + this.state.data.totalExpense) / this.state.data.noOfParticipant
            ...
            <div className="row wrapper border-bottom white-bg ">
                                                        ...
                                                        return (
                                                            ...
                                                            <tr key={index}>
                                                                ...
                                                                <td> <input
                                                                    ...
                                                                    disabled={isRequestedUser(this.state.user_id, this.state.data.createdBy) ? true : false}
                ...
                <div className="row col-md-12 margin-top-20">
                    ...
                    <div className="col-md-4">
                        ...
                        <div className="col-sm-12">
                            <input type="text" className="form-control" value={avgCostPerPerson.toFixed(2)}></input>
                        </div>
                    </div>
