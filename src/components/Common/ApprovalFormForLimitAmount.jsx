...
export default class ApprovalFormForLimitAmount extends Component {
            ...
            <div>
                {
                    this.checkCEO() ?
                        <div className="float-right m-b-10" >
                            ...
                            <div className="float-right m-b-10" >
                                <button className="btn btn_approved" onClick={() => this.props.approvalStatus('verified', this.state.comment)} disabled={this.props.status === 1 ? false : true} >
                                    <i className="fa fa-check"></i> Verify </button>
                                < button className="btn btn_reject"
                                    disabled={this.props.status === 1 ? false : true}
                                    onClick={this.showModal.bind(this)} > <i className="fa fa-times"></i> Reject </button >

                            </div > : checkHRAssistant() ?
                                <div className="float-right m-b-10" >
                                    <button className="btn btn_approved" onClick={() => this.props.approvalStatus('checked', this.state.comment)} disabled={this.props.status === 0 ? false : true} >
                                        <i className="fa fa-check"></i> Check </button>
                                    < button className="btn btn_reject"
                                        disabled={this.props.status === 0 ? false : true}
                                        onClick={this.showModal.bind(this)} > <i className="fa fa-times"></i> Reject </button >
                                ...
                                : this.haveAllPermission(permission) ?
                                    ...
                                    <div className="float-right m-b-10" >
                                        {permission.check_by > 0 ?
                                            <button className="btn btn-primary m-r-10" onClick={() => this.props.approvalStatus('checked', this.state.comment)} disabled={true}>
                                                <i className="fa fa-check"></i> Check </button>
                                            : permission.verify_by > 0 ?
                                                <button className="btn btn_verified" onClick={() => this.props.approvalStatus('verified', this.state.comment)} disabled={true}>
                                                    <i className="fa fa-check"></i> Verify </button>
                                                : permission.approve_by > 0 ?
                                                    <button className="btn btn_approved" onClick={() => this.props.approvalStatus('approved', this.state.comment)} disabled={true}>
                                                        <i className="fa fa-check"></i> Approve </button>
                                                    : ''}
                                        < button className="btn btn_reject"
                                            disabled={!this.checkRejectState() && check_limit_amount ? false : true}
