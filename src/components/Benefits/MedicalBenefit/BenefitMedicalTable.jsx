...
export default class BenefitMedicalTable extends Component {
    constructor(props) {
        super(props);
        this.state = {
    ...
    getAllBenefits() {
        let id = this.state.user_id;
        // fetch(main_url + "medical_benefit/getMedicalBenefit/" + id)
        fetch(main_url + "medical_benefit/getMedicalBenefit/" + id + "/" + moment(this.props.start_date).format("YYYY-MM-DD") + "/" + moment(this.props.end_date).format("YYYY-MM-DD"))
            .then(response => {
                if (response.ok) return response.json()
            })
            .then(res => {
    ...
    _setTableData = (data) => {
        ...
        for (var i = 0; i < data.length; i++) {
            ...
            obj = {
                no: i + 1,
                form_no: fno.fno_medical + data[i].form_no,
                employee_id: data[i].employment_id ? data[i].employment_id : [],
                employee_name: data[i].fullname ? data[i].fullname : [],
                position: data[i].designations ? data[i].designations : '-',
                branch: data[i].branch_name ? data[i].branch_name : [],
                beneficiary: data[i].type_name ? data[i].type_name : [],
                name_of_beneficiary: data[i].beneficary_name ? data[i].beneficary_name : [],
                amount: data[i].request_amount ? data[i].request_amount : [],
        ...
        var column = [
            ...
            { title: "Beneficiary", data: "beneficiary" },
            { title: "Name Of Beneficiary", data: "name_of_beneficiary" },
