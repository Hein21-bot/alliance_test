...
import { main_url, validate, getUserId, getDesignation, getLevel, getBranch, getDepartment } from '../../utils/CommonFunction';

var form_validate = true;
const workflow_type = [{ label: 'Work Flow', value: 1 }, { label: 'Work Flow Wiht Branch', value: 2 }]

export default class WorkFlowEditForMore
    extends Component {
    constructor(props) {
        super(props);
        this.state = {
            ...
            branch: [],
            department: [],
            selectedBranch: { label: props.data.data[0].branch_name, value: props.data.data[0].branch_id },
            selectedDept: { label: props.data.data[0].deptname, value: props.data.data[0].department_id },
        }
    }
    ...
    handleDepartment = event => {
        let data = this.state.data;
        data.department_id = event.value;
        this.setState({
            data: data,
            selectedDept: event
        })
    ...
    handleLimitAmount = event => {
        let data = this.state.data;
        data.over_limit_amount = event.target.checked;
        this.setState({
            data: data,
            over_limit_amount: event.target.checked
        })
    }
    ...
    check = () => {
        ...
        let status = 0;
        fetch(main_url + 'workflow/editWorkFlowWithAmount/' + data.workflow_id, {
            method: "POST",
            headers: {
                                ...
                                <div className="form-group">
                                    <div className="pretty p-default p-curve">
                                        <input type="checkbox" name="limitAmount"
                                            ...
                                            onChange={this.handleLimitAmount}
                                        />
                                        {/* <input type="checkbox" name="limitAmount"
                                            checked={this.state.data.over_limit_amount === 1 ? "checked" : ""}
                                        /> */}
